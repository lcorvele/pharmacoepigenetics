...
# Read in data
normalized_ab <- read.csv("C:/Users/lcorvele/R/Correlogram/Pharmacoepigenetics/PDX_sPTM_filtered.csv")

## CORRELATION PLOT

#Summarize rows by summing up all values if the column Peptide is the same
normalized_ab <- normalized_ab %>% group_by(PTM) %>% summarise(across(everything(), list(sum)))
...
write.csv(correlation_matrix_reordered, file = "correlation_matrix_PDX_filtered.csv")
...
correlation_matrix_csv <- read.csv("C:/Users/lcorvele/R/Correlogram/Pharmacoepigenetics/correlation_matrix_cell_lines_filtered.csv", row.names = 1)
...
write.csv(p_mat, file = "241217_pval_correlation_matrix_PDX.csv")
