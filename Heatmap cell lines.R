...
data_for_heatmap <- as.data.frame(read_excel("C:/Users/lcorvele/R/Heatmap/Pharmacoepigenetics/Baseline/Cell_lines_sPTM_data.xlsx", col_names = T, sheet = 2))
...
conditions <- sapply(strsplit(names(data_for_heatmap)[-1], "_"), function(x) x[3])
...
write.csv(average_data, "250613_PDX_sPTM_average_data.csv", row.names = TRUE)
...
data_for_annotation <- as.data.frame(read_excel("C:/Users/lcorvele/R/Heatmap/Pharmacoepigenetics/Baseline/Metadata_cell_lines.xlsx", col_names = T, sheet = 1))
# Ensure that data_for_annotation is a data frame
data_for_annotation <- as.data.frame(data_for_annotation)
colnames(data_for_annotation) <- c('Cell line', 'Subtype')
...
colours_subtype <- list('Subtype' = c('TAL1' = 'deeppink', 'TLX3' = 'cyan2', 'TLX1' = 'blueviolet', 'ETP' = 'chartreuse2', 'No' = 'darkgrey', 'LMO2' = 'gold'))
...
#create heatmap png
png("heatmap_cell_lines_sPTM_zscores_unmod_v2.png", height=1200, width=1200)
