...
```{r}
df <- df <- read.csv("C:/Users/lcorvele/R/msqrob2PTM/Pharmacoepigenetics/250611_Raw_Ab_PDX_short_doubles_removed_parsed.csv",header=TRUE,skip=0)
ecols = grep("221128", colnames(df))
#order dataframe by protein, for the normalisation step
df = df[order(df$Protein),]
df$peptidoform <- paste(df$Protein,df$Sequence, df$Variable.modifications...position..description., sep = "_")
...
colData(pe)$PDX <- sapply(str_split(rownames(colData(pe)), "_"), function(x) x[2])
colData(pe)$PDX <- as.factor(colData(pe)$PDX)
colData(pe)
```
...
rowData(pe[["peptidoform"]])$Master_Protein <- case_when(
  ...
  grepl(pattern = "H31", rowData(pe[["peptidoform"]])$Protein) ~ "H31_HUMAN",
...
limma::plotDensities(assay(pe[["peptidoformNorm"]]), col = as.numeric(colData(pe)$PDX))
```

```{r,}
boxplot(assay(pe[["peptidoformNorm"]]), col = colData(pe)$PDX,
...
pe <- msqrob2::msqrob(object = pe, i = "peptidoformNorm", formula = ~PDX, robust=FALSE, overwrite = T)
...
```{r}
contrasts <- c("PDX000.AZE", "PDXAA", "PDXCA", "PDXGA", "PDXJG", "PDXLDV", "PDXMaTil", "PDXMT", "PDXNH", "PDXNK", "PDXOS", "PDXPTST", "PDXVF")
...
```{r}
#We specify at least the object, the assay and the formula in the function
ptm <- msqrob2::msqrob(object = ptm, i = "ptm", formula = ~PDX, robust=TRUE, overwrite = T)
#We want to see the parameter names of the model
...
```{r}
write.csv(2^assay(pe[["peptidoformNorm"]]), "C:/Users/lcorvele/R/msqrob2PTM/Pharmacoepigenetics/250613_PDX_peptidoform_untransformed_data.csv")

write.csv(assay(pe[["peptidoformNorm"]]), "C:/Users/lcorvele/R/msqrob2PTM/Pharmacoepigenetics/250613_PDX_peptidoform_data.csv")

write.csv(ptm_x_assay, "C:/Users/lcorvele/R/msqrob2PTM/Pharmacoepigenetics/250613_PDX_sPTM_data.csv")

write.csv(2^ptm_x_assay, "C:/Users/lcorvele/R/msqrob2PTM/Pharmacoepigenetics/250613_PDX_sPTM_data_untransformed.csv")
```
